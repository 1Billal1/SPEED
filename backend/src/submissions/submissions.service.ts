...
export class SubmissionsService {
  ...
  ): Promise<Partial<CreateSubmissionDto>> {
    try {
      ...
      Object.keys(parsedData).forEach(
        (key) =>
          parsedData[key as keyof typeof parsedData] === undefined &&
  ...
  async create(
    createSubmissionDto: CreateSubmissionDto & { submitterId?: string },
  ): Promise<SubmissionDocument> {
    const submissionData: any = { ...createSubmissionDto, status: 'pending' };
    if (
      createSubmissionDto.submitterId &&
      Types.ObjectId.isValid(createSubmissionDto.submitterId)
    ) {
      submissionData.submitterId = new Types.ObjectId(
        createSubmissionDto.submitterId,
      );
    }
  ...
  async findById(id: string): Promise<SubmissionDocument | null> {
    if (!Types.ObjectId.isValid(id)) {
      console.warn(
        `SubmissionsService: findById called with invalid ID format: ${id}`,
      );
      return null;
    }
  ...
  async getPendingBySubmitter(
    submitterId: string,
  ): Promise<SubmissionDocument[]> {
    if (!Types.ObjectId.isValid(submitterId)) return [];
    if (!this.submissionModel.schema.path('submitterId')) {
      console.warn("SubmissionsService: 'submitterId' field does not exist.");
      return Promise.resolve([]);
    }
    return this.submissionModel
      .find({ submitterId: new Types.ObjectId(submitterId), status: 'pending' })
      .exec();
  }
  ...
  async editSubmission(
    ...
    if (submission.status !== 'pending')
      throw new ForbiddenException('Cannot edit non-pending submission.');
    if (!submission.editHistory) submission.editHistory = [];
    const submissionIdString =
      submission._id instanceof Types.ObjectId ||
      typeof submission._id?.toString === 'function'
        ? submission._id.toString()
        : null;
    if (!submissionIdString)
      throw new Error(`Critical error: Fetched submission has an invalid _id.`);
    console.log(
      `[SVC editSubmission] Editing submission with ID: ${submissionIdString}`,
    );
    const previousState = {
  ...
  async findSubmissionsByStatus(
    ...
    switch (statusQueryFromFrontend.toLowerCase()) {
      ...
      case 'rejected':
        ...
        break;
      case 'analyzed':
        dbStatusQuery = 'Analyzed';
        break;
      default:
        throw new BadRequestException(
          `Invalid status filter: ${statusQueryFromFrontend}`,
        );
    }
  ...
  async findPotentialDuplicates(
    ...
    if (doiToCheck && doiToCheck.trim() !== '') {
      ...
      const doiMatches = await this.submissionModel
        .find({
          _id: { $ne: new Types.ObjectId(submissionIdToExclude) },
          doi: {
            $regex: new RegExp(`^${this.escapeRegex(normalizedDoi)}$`, 'i'),
          },
          status: 'Accepted',
        })
        .limit(5)
      ...
      doiMatches.forEach((match) => {
        const plain = toPlainDuplicate(match, 1.0);
        if (!potentialDuplicatesMap.has(plain._id))
    ...
    if (titleToCheck && titleToCheck.trim() !== '') {
      ...
      if (normalizedTitleToCheck.length > 5) {
        const candidateSubmissions = await this.submissionModel
          .find({
            _id: { $ne: new Types.ObjectId(submissionIdToExclude) },
            status: 'Accepted',
        ...
        for (const existingDoc of candidateSubmissions) {
          if (existingDoc.title) {
            const normalizedExistingTitle = this.normalizeTitle(
              existingDoc.title,
            );
            if (normalizedExistingTitle.length > 5) {
              const similarity = stringSimilarity.compareTwoStrings(
                normalizedTitleToCheck,
              ...
              if (similarity >= SIMILARITY_THRESHOLD) {
                ...
                if (
                  !existingMapEntry ||
                  (existingMapEntry.similarityScore || 0) < similarity
                ) {
                  potentialDuplicatesMap.set(plain._id, plain);
                }
              }
  ...
  async moderateSubmission(
    ...
    if (
      submissionToUpdate.status !== 'pending' &&
      newStatus !== submissionToUpdate.status
    ) {
      console.warn(
        `Attempting to moderate submission ${id} not in 'pending' state. Current: ${submissionToUpdate.status}, New: ${newStatus}`,
      );
    }
