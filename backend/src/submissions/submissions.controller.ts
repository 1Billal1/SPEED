...
export class SubmissionsController {
  ...
  async getSubmissionDetailsForModeration(
    @Param('id') id: string,
  ): Promise<SubmissionDetailsWithDuplicates> {
    const submission: SubmissionDocument | null =
      await this.submissionsService.findById(id);
    ...
    if (submission.status === 'pending') {
      potentialDuplicates =
        await this.submissionsService.findPotentialDuplicates(
          submissionIdString,
          submission.title,
          submission.doi,
        );
  ...
  async moderateSubmission(
    @Param('id') id: string,
    @Body()
    body: { status: string; rejectionReason?: string; duplicateOfId?: string },
    ...
    if (!body.status || !validModerationStatuses.includes(body.status)) {
      throw new BadRequestException(
        `Invalid status provided for moderation. Must be one of: ${validModerationStatuses.join(', ')}`,
      );
    }
    if (
