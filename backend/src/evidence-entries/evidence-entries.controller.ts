...
import {
  EvidenceEntriesService,
  SearchEvidenceResult,
  SearchEvidenceParams,
} from './evidence-entries.service';
...
export class EvidenceEntriesController {
  constructor(
    private readonly evidenceEntriesService: EvidenceEntriesService,
  ) {}
  ...
  async create(
    ...
    if (!analystId || (analystId === placeholderAnalystId && !req.user?.id)) {
      console.warn(
        `[CTRL Evidence] WARNING: Using placeholder analyst ID: ${placeholderAnalystId}`,
      );
    }
    if (!analystId) {
      throw new BadRequestException('Analyst ID could not be determined.');
    }
    return this.evidenceEntriesService.create(
