      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          res.status(error.response.status).json(
            error.response.data || {
              message: `Error from backend for status ${status}`,
            }
          );
        } else {
          res.status(500).json({
            message: `No response from backend for status ${status}`,
          });
        }
        ...
        res.status(500).json({
          message: `Unknown error in proxy for /find-by-status (status: ${status})`,
        });
      }
    }
  } else {
